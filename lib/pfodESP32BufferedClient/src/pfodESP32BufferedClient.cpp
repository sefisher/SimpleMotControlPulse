...
void pfodESP32BufferedClient::setDebugStream(Print* out) {
  debugOut = out;
}

pfodESP32BufferedClient::pfodESP32BufferedClient() {
  client = NULL;
  debugOut = NULL;
...
pfodESP32BufferedClient* pfodESP32BufferedClient::connect(WiFiClient* _client) {
#ifdef DEBUG	
	if (debugOut) {
...
void pfodESP32BufferedClient::stop() {	
  sendAfterDelay();
  if (!client) {
    return;
  }
...
uint8_t pfodESP32BufferedClient::connected() {
	sendAfterDelay();
  if (!client) {
    return 0;
  }
...
size_t pfodESP32BufferedClient::write(const uint8_t *buf, size_t size) {
	sendAfterDelay();
  if (!client) {
    return 0;
  }
...
size_t pfodESP32BufferedClient::write(uint8_t c) {
	sendAfterDelay();
  if (!client) {
    return 0;
  }
...
int pfodESP32BufferedClient::availableForWrite() {
...
void pfodESP32BufferedClient::sendAfterDelay() {
...
void pfodESP32BufferedClient::forceSend(){
	if ((!client) || (!sendBufferIdx)) {  // common cases
    return;
  }
	if (client->connected()) {
...
int pfodESP32BufferedClient::available() {
  sendAfterDelay();
  if (!client) {
		return 0;
	}
...
int pfodESP32BufferedClient::read() {
  sendAfterDelay();
  if (!client) {
    return -1;
  }
...
int pfodESP32BufferedClient::peek() {
  sendAfterDelay();
  if (!client) {
    return -1;
  }
...
*/
void pfodESP32BufferedClient::flush() {
  forceSend();
